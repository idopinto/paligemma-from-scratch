...
from paligemma.common.embedding import Embedding
...
class SiglipVisionEmbeddings(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.n_positions = self.n_patches  # 196
        self.position_embedding = nn.Embedding(
            num_embeddings=self.n_positions, embedding_dim=config.hidden_size
        )
        self.register_buffer(
            "position_ids",
            torch.arange(self.n_positions).unsqueeze(0),  # [196] -> [1, 196]
            persistent=False,
        )
